...
const WasteRequest = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Strict check: only include if isActive is exactly true
  ...
  const handleViewLocation = (request) => {
    ...
    if (coords && Array.isArray(coords) && coords.length === 2) {
        // Leaflet requires [Latitude, Longitude], so we swap them
        // coords[1] is Lat, coords[0] is Lng
        const lat = coords[1];
        const lng = coords[0];

        // Basic validation to ensure they aren't default 0,0 if that matters
        if(lat !== 0 || lng !== 0) {
            setMapCoordinates([lat, lng]);
            setMapUserAddress(request.userId?.address || 'User Location');
            setOpenMapDialog(true);
        } else {
            toast.error("User coordinates are invalid (0,0).");
        }
    } else {
  ...
  const RequestCard = ({ request, statusType }) => (
    <div className="waste-request-card">
